...
#' @export
graphique_profil_horaire <- function(df) {
  prof <- df %>%
    dplyr::group_by(hour) %>%
    dplyr::summarise(
...
#' @return ggplot object
#' @export
graphique_profil_mensuel <- function(df) {
...
#' @return ggplot object
#' @export
graphique_serie_temporelle <- function(moyenne_journaliere) {
...
#' @return ggplot object
#' @export
graphique_top_capteurs <- function(stats, top = 10) {
  top_stats <- stats %>%
...
graphique_pm1_vs_pm25 <- function(df) {
  ...
  ggplot2::ggplot(sample, ggplot2::aes(x = pm1, y = pm25)) +
    ...
    ggplot2::labs(
      ...
      x = "PM1 (µg/m³)",
      y = "PM2.5 (µg/m³)",
      subtitle = sprintf("Corrélation = %.3f", cor_val)
    ) +
    style_plot()
}
...
graphique_correlation_capteurs <- function(df) {
  ...
  ggplot2::ggplot(corr_long, ggplot2::aes(x = sensor2, y = sensor1, fill = correlation)) +
    ...
    ggplot2::labs(
      title = "11. Matrice de Corrélation — Top 12 Capteurs",
      x = "Capteur",
      y = "Capteur"
    ) +
    style_plot() +
